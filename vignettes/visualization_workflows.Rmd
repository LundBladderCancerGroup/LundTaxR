...
This vignette provides a comprehensive guide to LundTaxR's visualization functions (excluding the main heatmaps, which are covered in the getting_started_with_LundTaxR). Each section describes the function's purpose, required and optional parameters, expected inputs, and output behavior, followed by practical examples with different parameter combinations. These tools help you interpret, present, and explore your classification results in a publication-ready format.
%% to be able to adjust function parameters I would need more info about the functions behind it and the parameteres them selves. maybe refer to to original packages behind it?
%% refer to ?xx() for more details about parameters? or link to function documentations online?
# Forest Plots
...
%% since you can start with precomputed data it would be good to show an example of that too? or how thte data should look like if you want to prepare it yourself.
**Key Parameters:** %% fix paragraphs
