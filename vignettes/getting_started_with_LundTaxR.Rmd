...
- **Signature score estimation**: Calculates additional signature scores for immune infiltration, stromal content, cell proliferation, and progression risk
...
- **Robust data handling**: Supports multiple gene ID formats and includes missing data imputation
  %%% with respect to data handling you could mention that it works with both RNA-seq and microarray data and that it is stable across different preprocessing pipelines

# Input Data Requirements

LundTaxR expects **gene expression data** in matrix or data frame format where: %% fix paragraphs below
...
LundTaxR includes the `sjodahl_2017` dataset - a curated expression matrix from Sjödahl et al. (2017) containing 307 bladder cancer samples. This microarray dataset serves as reference data for testing the classifier and is used in the examples below to demonstrate package functionality.
...
%% seems like you get info for 267 samples out of 307 in the dataset - maybe mention that?
...
LundTaxR allows the user to generate sophisticated heatmaps that visualize both the classification results and the underlying molecular signatures. The `plot_classification_heatmap()` function creates a multi-layered visualization showing:
...
%% is it poossible to customize which signatures to show in the heatmap? if so maybe mention that here
%% same for the signature heatmaps
%% if the user wants to just get the data used for plotting without plotting, is that possible?
...
This visualization complements the classification heatmap by visualizing additional key features of tumor biology, providing insights into bladder cancer heterogeneity beyond the molecular subtype itself.
