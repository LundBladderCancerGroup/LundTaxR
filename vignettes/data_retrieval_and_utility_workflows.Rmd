...
You can specify the samples to keep either by providing a vector of sample IDs, or by passing a metadata data frame that contains the desired samples(%% this is also based on a var "sample_ID"? maybe simplify this part). The function will return a new predictions list for the subset of samples specified.
...
%% this is ambiguous - maybe mention the structure of the metadata or refer to where you can see that and the variables available for filtering?
...
%% why not show how to sort the returned data using the ratio or sample order function?
...
```
%% I was wondering if you can do continuous variables too, like average age? also could you get something like table(df$subtype, df$gender)?
